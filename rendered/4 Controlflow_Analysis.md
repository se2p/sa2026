...
class CFGBuilder(CFGBuilder):
    ...
    def add_switch_node(self, switch_node: javalang.tree.SwitchStatement):
        ...
        for case in switch_node.cases:
            ...
            # If the case doesn't end in a break, it 'falls through' 
            # to the next case (frontier remains populated).
            if case.statements and isinstance(case.statements[-1], javalang.tree.BreakStatement):
                break_cases.extend(self.frontier)
                self.frontier = []

        # If there's no default case, the switch header can skip 
        # directly to the end of the switch block.
        if not has_default:
            self.frontier.append(cfg_node)
